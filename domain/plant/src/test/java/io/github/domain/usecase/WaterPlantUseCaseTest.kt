...
class WaterPlantUseCaseTest {
  ...
  fun testWateringNewPlantSameWeek() = runTest {
    ...
    assertEquals(
        expected = LocalDateTime.of(2023, Month.DECEMBER, 27, 8, 0),
        actual = result.getOrThrow().nextWaterDate)
  }
  ...
  fun testWateringNewPlantNextWeek() = runTest {
    ...
    assertEquals(expected = now, actual = result.getOrThrow().lastWaterDate)
    assertEquals(
        expected = LocalDateTime.of(2024, Month.JANUARY, 1, 8, 0),
        actual = result.getOrThrow().nextWaterDate)
  }
  ...
  fun testWateringOldPlant() = runTest {
    ...
    assertEquals(expected = now, actual = result.getOrThrow().lastWaterDate)
    assertEquals(
        expected = LocalDateTime.of(2024, Month.JANUARY, 15, 8, 0),
        actual = result.getOrThrow().nextWaterDate)
  }
